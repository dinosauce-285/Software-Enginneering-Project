    ...
    <>
      <div className="flex items-center gap-1">
        <div className="relative" ref={avatarMenuRef}>
          <div
            onClick={isRegularUser ? () => setIsAvatarMenuOpen(!isAvatarMenuOpen) : undefined}
            title={isRegularUser ? "Avatar Menu" : "Admin Avatar"}
            className={`flex items-center justify-center ${isRegularUser ? 'cursor-pointer' : 'cursor-default'}`}
          >
            <img
              src={user.avatar || defaultAvatar}
              alt="User Avatar"
              className="w-9 h-9 rounded-full object-cover bg-gray-200 transition-opacity hover:opacity-80"
            />
          </div>

          {isRegularUser && isAvatarMenuOpen && (
