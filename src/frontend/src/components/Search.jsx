                ...
                {isExpanded && (
                    <div className="absolute top-full left-0 right-0 z-20 bg-white border border-gray-200 shadow-lg rounded-lg p-4 mt-2 animate-fade-in-down">
                        <div className="flex flex-col sm:flex-row items-center gap-3">
                            ...
                            <div ref={emotionDropdownRef} className="relative w-full sm:flex-1">
                                <button onClick={() => setEmotionDropdownOpen(prev => !prev)} className="w-full flex items-center justify-between pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-left focus:ring-blue-500 focus:border-blue-500 focus:outline-none">
                                    <span className={selectedEmotions.length > 0 ? "text-gray-800" : "text-gray-500"}>{getEmotionButtonText()}</span>
                                    <ChevronDownIcon />
                                </button>
                                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><EmotionIcon /></div>
                                {isEmotionDropdownOpen && (
